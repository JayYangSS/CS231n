...
class KNearestNeighbor:
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        #####################################################################
        dists[i][j]=np.linalg.norm(self.X_train[j]-X[i],2)#calculate the L2 distance
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
    return dists
  ...
  def compute_distances_no_loops(self, X):
    ...
    # the method above will cause memoryError, the following method use the complete square formula expansion.
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      sortIndex=np.argsort(dists[i,:])
      closest_y=self.y_train[sortIndex[0:k]]
      #########################################################################
      # TODO:                                                                 #
